#!/usr/bin/env node
/**
 * Independent Square API Test Script
 *
 * This script allows you to test Square API calls directly without Supabase.
 * It includes both live API testing and mock data testing.
 *
 * Usage:
 *   node test-square-api.js --help
 *   node test-square-api.js --mock
 *   node test-square-api.js --live --token YOUR_ACCESS_TOKEN
 *
...
// Configuration
const config = {
  environment: SquareEnvironment.Sandbox, // Change to SquareEnvironment.Production for live testing
  applicationId: "", // Your Square Application ID (optional for most operations)
};
...
class SquareAPITester {
  constructor(accessToken, environment = SquareEnvironment.Sandbox) {
    if (accessToken) {
  ...
  async testMerchantInfo() {
    ...
    try {
      ...
      log(`❌ Error fetching merchant info: ${error.message}`, colors.red);
      throw error;
    }
  }
  ...
  async testLocations() {
    ...
    try {
      ...
      log(`❌ Error fetching locations: ${error.message}`, colors.red);
      throw error;
    }
  }
    ...
    try {
      ...
      if (response.result.objects) {
        ...
        const items = objects.filter(
          (obj) => obj.itemData && !this.isService(obj)
        );
        const services = objects.filter(
          (obj) => obj.itemData && this.isService(obj)
        );
  ...
  isService(catalogObject) {
    if (!catalogObject.itemData || !catalogObject.itemData.variations) {
      return false;
    }

    return catalogObject.itemData.variations.some(
      (variation) =>
  ...
  async runFullTest() {
    log("🧪 Starting Square API Test Suite", colors.bright + colors.blue);
...
// CLI handling
function showHelp() {
  log("Square API Independent Test Script", colors.bright + colors.blue);
  log("\nUsage:", colors.bright);
  log("  node test-square-api.js [options]", colors.cyan);
  ...
  log("  node test-square-api.js --mock", colors.cyan);
  log(
    "  node test-square-api.js --live --token YOUR_ACCESS_TOKEN",
    colors.cyan
  );
  log(
    "  node test-square-api.js --live --token YOUR_ACCESS_TOKEN --production",
    colors.cyan
  );
...
process.on("unhandledRejection", (error) => {
  log(`💥 Unhandled error: ${error.message}`, colors.bright + colors.red);
  process.exit(1);
});
